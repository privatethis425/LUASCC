...
local colors = {
    ...
    NearestOn = Color3.fromRGB(60, 200, 110),
    NearestOff = Color3.fromRGB(100, 100, 120)
}
...
local function startCombat()
    ...
    combatHeartbeat = runService.Heartbeat:Connect(function()
        if nearestEnabled then
            ...
            if nearest ~= selectedTarget then
                selectedTarget = nearest
                if selectedTarget then
                    if playerDropdown then
                        playerDropdown.Text = selectedTarget.DisplayName .. " (@" .. selectedTarget.Name .. ")"
                    end
                    ...
                    restoreTargetHitbox()
                    if playerDropdown then playerDropdown.Text = "No Nearest Target" end
                end
                updateStatus()
            end
        end
        ...
        if not target or not target.Parent then
            ...
            if nearestEnabled then
                if playerDropdown then playerDropdown.Text = "Finding Nearest..." end
                updateStatus()
                return
            end
...
-- ========================== MOBILE OPEN/CLOSE FLOATING ICON ==========================
local mobileToggleButton = Instance.new("TextButton")
mobileToggleButton.Name = "MobileGuiToggle"
mobileToggleButton.Size = UDim2.fromOffset(56, 56)
mobileToggleButton.Position = UDim2.new(1, -72, 0.5, -28)
mobileToggleButton.AnchorPoint = Vector2.new(0, 0)
mobileToggleButton.BackgroundColor3 = colors.Panel
mobileToggleButton.BorderSizePixel = 0
mobileToggleButton.Text = "X"
mobileToggleButton.TextColor3 = colors.TextPrimary
...
mobileToggleStroke.Color = colors.Stroke
mobileToggleStroke.Thickness = 1.5
mobileToggleStroke.Parent = mobileToggleButton

local mobileGuiOpen = true
local mobileDragging = false
local mobileMoved = false
local mobileDragInput = nil
local mobileDragStart = nil
local mobileStartAbsolute = nil
local MOBILE_DRAG_THRESHOLD = 8
local MOBILE_EDGE_PADDING = 8

local function setMobileGuiOpen(isOpen)
    mobileGuiOpen = isOpen
    mainFrame.Visible = isOpen
    mobileToggleButton.Text = isOpen and "X" or "≡"
end
...
mobileToggleButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        mobileStartAbsolute = mobileToggleButton.AbsolutePosition
    end
end)

UserInputService.InputChanged:Connect(function(input)
    if not mobileDragging or input ~= mobileDragInput or not mobileDragStart or not mobileStartAbsolute then
        return
    end

    local delta = input.Position - mobileDragStart
    if delta.Magnitude >= MOBILE_DRAG_THRESHOLD then
        mobileMoved = true
    end
    ...
    if mobileMoved then
        ...
        local newX = math.clamp(
            mobileStartAbsolute.X + delta.X,
            MOBILE_EDGE_PADDING,
            math.max(MOBILE_EDGE_PADDING, viewportSize.X - buttonSize.X - MOBILE_EDGE_PADDING)
        )
        local newY = math.clamp(
            mobileStartAbsolute.Y + delta.Y,
            MOBILE_EDGE_PADDING,
            math.max(MOBILE_EDGE_PADDING, viewportSize.Y - buttonSize.Y - MOBILE_EDGE_PADDING)
        )
        mobileToggleButton.Position = UDim2.fromOffset(newX, newY)
    end
end)

UserInputService.InputEnded:Connect(function(input)
    if input ~= mobileDragInput then return end

    local wasMoved = mobileMoved
    mobileDragging = false
    mobileMoved = false
    mobileDragInput = nil
    mobileDragStart = nil
    mobileStartAbsolute = nil

    if not wasMoved then
        setMobileGuiOpen(not mobileGuiOpen)
    end
end)
            ...
            btn.MouseButton1Click:Connect(function()
                if nearestEnabled then
                    ...
                    if nearestButton then
                        nearestButton.Text = "Nearest: Off"
                        TweenService:Create(nearestButton, TweenInfo.new(0.3), {BackgroundColor3 = colors.NearestOff}):Play()
                    end
                end
                selectedTarget = plr
                if hitboxEnabled then
...
nearestButton = Instance.new("TextButton")
nearestButton.BackgroundColor3 = colors.NearestOff
nearestButton.Text = "Nearest: Off"
nearestButton.TextColor3 = colors.TextPrimary
nearestButton.TextSize = 12
nearestButton.Font = Enum.Font.GothamBold
nearestButton.Parent = buttonGrid
local cornerNearest = Instance.new("UICorner")
cornerNearest.CornerRadius = UDim.new(0, 6)
cornerNearest.Parent = nearestButton
applyButtonAnimation(nearestButton)
...
local function toggleFollow()
    ...
    if not followEnabled and nearestEnabled then
        nearestEnabled = false
        if nearestButton then
            nearestButton.Text = "Nearest: Off"
            TweenService:Create(nearestButton, TweenInfo.new(0.3), {BackgroundColor3 = colors.NearestOff}):Play()
...
local function toggleNearestAction()
    ...
    if nearestEnabled then
        ...
        if not nearest then
            nearestEnabled = false
            notify("No Target", "No alive player is available nearby.")
            return
        end
        ...
        nearestButton.Text = "Nearest: On"
        TweenService:Create(nearestButton, TweenInfo.new(0.3), {BackgroundColor3 = colors.NearestOn}):Play()
        ...
        startCombat()
        notify("Nearest Target Enabled", "LoopTP will automatically follow the nearest player.")
    else
        nearestButton.Text = "Nearest: Off"
        TweenService:Create(nearestButton, TweenInfo.new(0.3), {BackgroundColor3 = colors.NearestOff}):Play()
        followEnabled = false
...
nearestButton.MouseButton1Click:Connect(toggleNearestAction)
...
updateStatus = function()
    ...
    if nearestEnabled then
        table.insert(parts, "Nearest: Active")
    end
