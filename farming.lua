...
task.spawn(function()
	local optimizerOk, optimizerErr = pcall(function()
		...
		local function isStreakOrSkillRelated(instance)
		    ...
		    if instance:IsA("Attribute") then
		        return true
		    end
		...
		local function isCoreCharacterPart(instance)
		    ...
		    -- The character you're currently playing is never touched, in any way -
		    -- clothes, accessories, skill VFX, streak display, camera-linked rig
		    -- parts. Everything on it stays exactly as the game spawned it.
		...
		local function cleanInstance(instance)
		    ...
		    -- Full bypass: never mark, count, or touch anything under our own
		    -- character. This is what keeps its camera animation and all its
		    -- effects alive - it's not run through cleanCharacterItem at all,
		    -- not even the "disable visual" path.
		    if isLocalPlayerCharacter(instance) then
		        return
		    end
		    ...
		    -- Streak/skill protection must run BEFORE the camera-VFX path - streak
		...
		print("[TSB Ultra Baseplate Optimizer] smooth mode active (camera/anim-safe); run again to safely restart")
	end)
...
local function setupAntiAfk()
	localPlayer2.Idled:Connect(function()
		-- Do not simulate mouse input: on mobile executors this can spawn a cursor.
		pcall(function()
